...
<ul>
  <li>
    <p>
		Se in corrispondenza del paniere $(M/P_X,0)$ dove il reddito è speso interamente in $X$, cioè l'intercetta orizzontale
		<span class="marginnote">
		...
		del vincolo di bilancio, abbiamo $MU_X/P_X>MU_Y/P_Y$, allora la scelta ottima è il paniere $(M/P_X,0)$. Il consumatore ha un incentivo a ridurre il consumo di $Y$, ma non può farlo, essendo esso già zero.
		</p>
	</li>
  <li>
    <p>
		Se in corrispondenza del paniere $(0,M/P_Y)$ dove il reddito è speso interamente in $Y$, cioè l'intercetta verticale
		<span class="marginnote">
		...
		del vincolo di bilancio, abbiamo $MU_X/P_X< MU_Y/P_Y$, allora la scelta ottima è il paniere $(0,M/P_Y)$. Il consumatore ha un incentivo a ridurre il consumo di $X$, ma non può farlo, essendo esso già zero.
		</p>
	</li>
  <li>
    <p>
		Se non siamo in nessuna delle due situazioni precedenti, allora la scelta ottima è un paniere interno, che possiamo calcolare risolvendo un sistema di due equazioni (in due incognite, ovvero le quantità dei due beni). La prima equazione è il vincolo di bilancio. La seconda richiede che $MU_X/P_X$ non sia né maggiore né minore di $MU_Y/P_Y$, cioè che si abbia <i>tangenza</i> tra curva di indifferenza e vincolo di bilancio:
		<p><span style="color: Blue;">
		\(\begin{gathered}
		 MRS_{XY}=P_X/P_Y
		\end{gathered}\)
...
Ora che sappiamo calcolare la scelta ottima, possiamo anche facilmente calcolare la <i><b>funzione di domanda individuale</b></i> di ciascun bene. Una funzione di domanda individuale è la relazione tra il prezzo di un bene e la quantità domandata di quel bene da parte di un singolo consumatore a quel prezzo, tenendo fissi reddito e prezzo dell'altro bene. Descrivere questo legame è utile se siamo interessati (e lo saremo nel resto di queste note) a parlare del mercato di un singolo bene alla volta e la questione che vogliamo focalizzare è quindi come cambia il consumo di <i>quel</i> bene quando il prezzo di <i>quel</i> bene aumenta o diminuisce. Un altro vantaggio è la semplificazione: si evidenzia la relazione tra la quantità domandata di un bene ed il suo prezzo attraverso un concetto facilmente fruibile anche da chi (diversamente da noi 🤓) non possiede tutto l'armamentario di utilità, vincolo di bilancio, ecc.
