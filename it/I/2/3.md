...
Siamo adesso pronti a definire in modo preciso e calcolare la scelta ottima del consumatore, ovvero il paniere preferito tra quelli che può permettersi. Per brevità, chiameremo le coordinate di (cioè le quantità dei beni in) quel paniere le quantità <i><b>domandate</b></i> dei due beni.

Come illustriamo nella figura qui sotto, la scelta ottima è il paniere che soddisfa le seguenti due proprietà: primo, il paniere è accessibile; secondo, l'insieme dei panieri ad esso preferiti non si sovrappone all'insieme dei panieri accessibili.
...
Come si nota dalla figura, l'ottimalità richiede che il consumatore spenda tutto il proprio reddito (questa è ovviamente una conseguenza della non sazietà)
...
La prima equazione ci dice quale deve essere la somma delle spese per i due beni, la seconda quale deve essere il loro rapporto. Da queste due informazioni è facile calcolare quanto il consumatore spende in ciascuno dei due beni
...
Assumere preferenze
<span class="marginnote">
Nella <a href="{{ site.baseurl }}/it/I/2/3#gr_consumer/cobb-overlap">Figura 2.13</a>, per esempio, con $a=2$, $b=1$, $M=360$, $P_X=40$ e $P_Y=40$, la scelta ottima è spendere due terzi del reddito (ovvero $(2/3)\times 360=240$) in $X$ (comprandone quindi $240/40=6$ unità) e un terzo (ovvero $(1/3)\times 360=120$) in $Y$ (comprandone quindi $120/40=3$ unità).
</span>
...
<ul>
  <li>
    <p>
		La quantità domandata di ciascuno dei due beni aumenta all'aumentare del reddito: $M$ appare al numeratore sia nella formula di $X$ che in quella di $Y$. Gli economisti definiscono <i><b>normale</b></i> un bene la cui quantità domandata
		<span class="marginnote">
		In queste note consideriamo solo beni normali.
		</span>
		aumenta all'aumentare del reddito, e <i><b>inferiore</b></i> uno la cui quantità diminuisce all'aumentare del reddito.
		</p>
	</li>
  <li>
    <p>
		La quantità domandata di ciascuno dei due beni è indipendente dal prezzo dell'altro
		<span class="marginnote">
		...
		bene: $P_Y$ non appare nella formula di $X$ e, viceversa, $P_X$ non appare nella formula di $Y$.
		Gli economisti
